...
for(cnt.i in prj.descriptors.col.ids){
  ...
  tab.2 <- rbind(tab.2, c("", ""))
  tab.2 <- rbind(tab.2, c(prj.descriptors.descriptions[cnt.j], ""))
  tab.2 <- rbind(tab.2, cbind(names(table(df.music[cnt.i])), paste0(100*round(prop.table(table(df.music[cnt.i])),2)," %")))
}
hlines  <- c(hlines, nrow(tab.2))


colnames(tab.2) <- c("Clusters", "Sample")
...
print(tab.2 , tabular.environment="tabularx" ,width="0.5\\textwidth" ,include.rownames=FALSE, hline.after = hlines, comment=F, booktabs=T)
