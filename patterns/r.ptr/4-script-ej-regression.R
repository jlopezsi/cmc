...
str(data.dummies)
oci.one.b<-glm(One~Genre*Education+SizeTown+Age*Education+Language*ParentsEducation+Region, weights=ej12_8$Pes, family = binomial(logit), data=data.dummies)
library(Zelig)

oci.one.z<-zelig(One~Genre*Education+SizeTown+Age*Education+Language*ParentsEducation+Region,  weights= ej12_8$Pes,  model = "logit", data=data.dummies)
oci.two.z<-zelig(Two~Genre*Education+SizeTown+Age*Education+Language*ParentsEducation+Region,  weights= ej12_8$Pes,  model = "logit", data=data.dummies)
oci.three.z<-zelig(Three~Genre*Education+SizeTown+Age*Education+Language*ParentsEducation+Region,  weights= ej12_8$Pes,  model = "logit", data=data.dummies)
oci.four.z<-zelig(Four~Genre*Education+SizeTown+Age*Education+Language*ParentsEducation+Region,  weights= ej12_8$Pes,  model = "logit", data=data.dummies)


library(stargazer)
stargazer(oci.one.z, oci.two.z, oci.three.z, oci.four.z, no.space = TRUE, type='text',  title="Regression Results ")
...
oci.mn1<-multinom(as.factor(oci_en.Mclust$classification)~Genre+Age+SizeTown+Region, weights=ej12_8$Pes, data=oci.ilu1_en)
oci.mn1b<-multinom(as.factor(oci_en.Mclust$classification)~Genre+Age+SizeTown, weights=ej12_8$Pes, data=oci.ilu1_en)
oci.mn1c<-multinom(as.factor(oci_en.Mclust$classification)~Genre+Age, weights=ej12_8$Pes, data=oci.ilu1_en)
...
oci.mn2<-multinom(as.factor(oci_en.Mclust$classification)~Genre+Age*Education+Region+SizeTown, weights=ej12_8$Pes, data=oci.ilu1_en)
...
oci.mn3<-multinom(as.factor(oci_en.Mclust$classification)~Genre*Education+Education*Age+Region+SizeTown, weights=ej12_8$Pes, data=oci.ilu1_en)
...
anova(oci.mn1, oci.mn1b)
anova(oci.mn1b, oci.mn1c)
stargazer(anova(oci.mn0, oci.mn1), no.space = TRUE, type='text',  title="Resting models 0 & 1")
anova(oci.mn1b, oci.mn2)
...
stargazer(anova(oci.mn3, oci.mn4), no.space = TRUE, type='text',  title="Resting models 3 & 4")
...
stargazer(oci.mn4, no.space = TRUE, out = "m4.pdf", title="Regression Results M4")
stargazer(oci.mn44, no.space = TRUE, type='text',  title="Regression Results M4")
